    ...
    return (
      <div className="topic-list">
        ...
        <Table dataSource={topicList} rowKey="type" pagination={false}>
          <Column align="center" title="题目类型" key="type" dataIndex="type" render={(text, record) => (
            <Link to={`/exam/${text}/${examId}`}>{topicType[text]}</Link>
          )}/>
          <Column align="center" title="题目数量" data="topicCount" dataIndex="topicCount"/>
          <Column align="center" title="完成数量" data="answerCount" dataIndex="answerCount"/>
        </Table>
      </div>
    );
  }
